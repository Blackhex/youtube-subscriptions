...
- **Do not infer categories from "value is a list".** PocketTube keeps internal caches at the top level as plain lists — `liveStreamsCurrent` (channel ids), `nvl` (recent video *objects*) and `nvlo` (bare video ids) — and they leak in as bogus categories. Use PocketTube's registry instead: the union of the keys of `ysc_collection` and `ysc_meta` (both keyed by category name, each used only when it is a `dict`); a list key is a category only if its `_ysm_\d+`-stripped base name is in that union. In a real 65-key dump this separated 43 category keys from the 4 internals exactly. Fall back to the deny-list rule only when both maps are missing.
- Two traps when tightening this filter: **empty categories are real** (a user category with no channels is `[]` and appears in the registry — don't drop it), and **a category list may contain junk entries** that are not channel ids (real dumps have a few) — never require every element to be a channel id, that discards whole categories.
- **`ysc_meta[name]['position']` carries PocketTube's display order** and is a *single global depth-first sequence* (`0..N-1`, contiguous, no duplicates in a real 40-category dump): each parent is immediately followed by its own children. So it can be assigned directly to `sort_order` — no per-parent renumbering — and it orders the root level and each sibling group at once. Look it up by the `_ysm_\d+`-stripped base name, the same name used for the registry, and guard for `ysc_meta` missing/non-dict, the entry missing/`None`/non-dict, and a non-int position (`isinstance(x, int) and not isinstance(x, bool)`). Put it in `get_or_create(defaults=...)` so additive mode only sets it on rows it creates.
- **A category with no usable position must sort after every positioned one.** Real dumps contain `ysc_meta[name] = null` (key present, no position), and falling back to the sibling count gives a *small* number that interleaves it among the positioned categories — one such entry sorted 2nd of 12 roots instead of last. Assign `max(position) + 1 + k` with `k` a counter incremented in payload-encounter order (`Category.Meta.ordering = ['sort_order', 'name']` would otherwise tie-break alphabetically). Keep the sibling-count fallback only for dumps whose `ysc_meta` carries no positions at all.
